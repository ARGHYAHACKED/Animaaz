...
const AdminCurationForYou: React.FC = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        const [curRes, allRes] = await Promise.all([
          api.get('/curation/forYou').catch(err => {
        ...
        setForYou(curRes.data.animeIds || []);
  ...
  const handleAdd = () => {
    ...
    if (anime && !forYou.some(f => f._id === anime._id)) {
      setForYou([...forYou, anime]);
      setSelectedToAdd('');
    }
  };

  const handleRemove = (id: string) => {
    setForYou(forYou.filter(a => a._id !== id));
  };
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      const animeIds = forYou.map(a => a._id);
      await api.put('/curation/forYou', { animeIds });
    } catch (err: any) {
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-8 pb-4 border-b border-gray-200">
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900">For You</h1>
          <div className="text-gray-500 mt-1 text-sm">Manage the curated list for For You Anime.</div>
        </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        ...
        {loading ? (
          ...
          <>
            {forYou.length === 0 && (
              <div className="mb-4 text-blue-600">No For You curation yet. Add anime and save to create it.</div>
            )}
            ...
            <div className="mt-6 flex justify-end">
              <button
                ...
                disabled={saving}
              >
                {saving ? 'Saving...' : 'Save For You List'}
              </button>
            </div>
          </>
        )}
...
export default AdminCurationForYou;
