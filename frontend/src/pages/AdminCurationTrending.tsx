...
const AdminCurationTrending: React.FC = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        const [curRes, allRes] = await Promise.all([
          api.get('/curation/trending').catch(err => {
        ...
        setTrending(curRes.data.animeIds || []);
  ...
  const handleAdd = () => {
    ...
    if (anime && !trending.some(f => f._id === anime._id)) {
      setTrending([...trending, anime]);
      setSelectedToAdd('');
    }
  };

  const handleRemove = (id: string) => {
    setTrending(trending.filter(a => a._id !== id));
  };
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      const animeIds = trending.map(a => a._id);
      await api.put('/curation/trending', { animeIds });
    } catch (err: any) {
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-8 pb-4 border-b border-gray-200">
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900">Trending</h1>
          <div className="text-gray-500 mt-1 text-sm">Manage the curated list for Trending Anime.</div>
        </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        ...
        {loading ? (
          ...
          <>
            {trending.length === 0 && (
              <div className="mb-4 text-blue-600">No Trending curation yet. Add anime and save to create it.</div>
            )}
            ...
            <div className="mt-6 flex justify-end">
              <button
                ...
                disabled={saving}
              >
                {saving ? 'Saving...' : 'Save Trending List'}
              </button>
            </div>
          </>
        )}
...
export default AdminCurationTrending;
