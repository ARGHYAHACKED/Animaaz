...
const AdminCurationTopWeek: React.FC = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        const [curRes, allRes] = await Promise.all([
          api.get('/curation/topWeek').catch(err => {
        ...
        setTopWeek(curRes.data.animeIds || []);
  ...
  const handleAdd = () => {
    ...
    if (anime && !topWeek.some(f => f._id === anime._id)) {
      setTopWeek([...topWeek, anime]);
      setSelectedToAdd('');
    }
  };

  const handleRemove = (id: string) => {
    setTopWeek(topWeek.filter(a => a._id !== id));
  };
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      const animeIds = topWeek.map(a => a._id);
      await api.put('/curation/topWeek', { animeIds });
    } catch (err: any) {
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-8 pb-4 border-b border-gray-200">
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900">Top This Week</h1>
          <div className="text-gray-500 mt-1 text-sm">Manage the curated list for Top This Week Anime.</div>
        </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        ...
        {loading ? (
          ...
          <>
            {topWeek.length === 0 && (
              <div className="mb-4 text-blue-600">No Top This Week curation yet. Add anime and save to create it.</div>
            )}
            ...
            <div className="mt-6 flex justify-end">
              <button
                ...
                disabled={saving}
              >
                {saving ? 'Saving...' : 'Save Top This Week List'}
              </button>
            </div>
          </>
        )}
...
export default AdminCurationTopWeek;
