// src/components/Admin/ForYouManager.tsx
import React, { useEffect, useState } from 'react';
import api from '../../../utils/api';
import toast from 'react-hot-toast';
import { Loader2 } from 'lucide-react';

const ForYouManager: React.FC = () => {
    // --- Configuration for this specific component ---
    const key = 'forYou';
    const title = 'For You Selection';
    const badgeColor = 'bg-pink-500';
    const maxItems = undefined; 

    // --- State & Logic (Same as TrendingManager.tsx) ---
                    ...
                    <div className="mt-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {selectedItems.slice(0, 6).map((id:string) => {
                                ...
                                return (
                                    <div key={id} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-pink-50 border border-pink-200">
                                        {item?.coverImage && <img src={item.coverImage} alt="" className="w-6 h-6 rounded object-cover" />}
                                        <span className="text-xs font-medium text-pink-700 truncate max-w-[120px]" title={item?.title || id}>{item?.title || id}</span>
                                    </div>
                                );
                            })}
