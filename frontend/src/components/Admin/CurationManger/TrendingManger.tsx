...
const TrendingManager: React.FC = () => {
    ...
    const maxItems = undefined; // No max limit for trending
    ...
    const loadOptions = async (q = '') => {
        setLoading(true);
        try {
            const res = await api.get('/admin/anime/options', { params: { search: q } });
            setOptions(res.data);
        } catch (error) {
            toast.error('Failed to load anime options.');
        } finally {
            setLoading(false);
        }
    };

    // Load initial state and debounce search
    useEffect(() => {
        loadOptions('');
        (async () => {
            try {
                // Fetch the state for this specific section only
    ...
    useEffect(() => {
        const timer = setTimeout(() => loadOptions(searchQ), 400);
    ...
    const saveChanges = async () => {
        ...
        try {
            const payload: any = {};
            // Set the payload key dynamically: trendingIds
            payload[`${key}Ids`] = selectedItems;
                    ...
                    <div className="mt-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {selectedItems.slice(0, 6).map((id:string) => {
                                ...
                                return (
                                    <div key={id} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-purple-50 border border-purple-200">
                                        {item?.coverImage && <img src={item.coverImage} alt="" className="w-6 h-6 rounded object-cover" />}
                                        <span className="text-xs font-medium text-purple-700 truncate max-w-[120px]" title={item?.title || id}>{item?.title || id}</span>
                                    </div>
                                );
                            })}
