// src/components/Admin/CurationManger/BannerManager.tsx
...
const BannerManager: React.FC = () => {
    // --- Configuration ---
    const key = 'banner';
    const title = 'Homepage Banner (Max 5)';
    ...
    const saveChanges = async () => {
        ...
        try {
            ...
            payload[`${key}Ids`] = selectedItems; // Payload: { bannerIds: [...] }
                        ...
                        <label key={o._id} className="flex items-center gap-3 text-sm text-gray-900 bg-white rounded-lg px-3 py-2 hover:bg-gray-50 cursor-pointer transition-colors">
                            <input 
                                ...
                                onChange={(e:any) => {
                                    ...
                                    const finalSelection = maxItems ? next.slice(0, maxItems) : next;
                                    setSelectedItems(finalSelection);

                                    // Optional: Add toast for max limit reached
                                    if (isChecked && maxItems && finalSelection.length >= maxItems) {
                                        toast.error(`Maximum of ${maxItems} items allowed for ${title}.`);
                                    }
                                }} 
                    ...
                    <div className="mt-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {selectedItems.slice(0, 6).map((id:string) => {
                                ...
                                return (
                                    <div key={id} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-blue-50 border border-blue-200">
                                        {item?.coverImage && <img src={item.coverImage} alt="" className="w-6 h-6 rounded object-cover" />}
                                        <span className="text-xs font-medium text-blue-700 truncate max-w-[120px]" title={item?.title || id}>{item?.title || id}</span>
                                    </div>
                                );
                            })}
